...
On a native `mship` install on macOS (arm64) or Linux (x86_64/aarch64), `MSHIP_LLAMA_SERVER_BIN` is auto-provisioned (downloaded, sha256-verified, and cached under `~/.modelship/cache/llama.cpp/`) — see `modelship/launcher.py`'s `_resolve_llama_server_bin` and its `_LLAMA_CPP_ASSETS` table. Set `MSHIP_LLAMA_SERVER_BIN` yourself to skip this and use your own binary.
